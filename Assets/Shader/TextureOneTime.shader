	...
	SubShader{
			...
			Pass{
				...
				struct v2f {
					...
					float4  fixedPos : TEXCOORD2;	// the 3rd UV coordinate
				};

				//顶点程序和片段程序中用来计算UV的匹配和最护模型效果的渲染
				v2f vert(appdata_base v)  //appdata_base是包含於UnityCG.cginc中的結構體，顶点由位置(POSITION)、法线(NORMAL)和一个纹理坐标(TEXCOORD0)构成。 
				{
					v2f o;
					o.pos = mul(UNITY_MATRIX_MVP, v.vertex);
					/*
					M: World Matrix - Transforms 3D data from Model Space into World Space.
					V: View Marix - Transforms from World Space into View Space.
					P:  Projection Matrix - Transforms from View Space into Screen Space. (orthogonal/perspective)
					*/
					o.uv = TRANSFORM_TEX(v.texcoord, _MainTex); //根据mesh上的uv坐标来计算真正的纹理上对应的位置

					//Lerp does a linear interpolate between two values. lerp(a,b,w) = a+w*(b-a)
					...
					o.fixedPos = ComputeScreenPos(mul(UNITY_MATRIX_VP, fixedPos)); //获得一个投影点对应的屏幕坐标点
					return o;
				}
